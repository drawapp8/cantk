...
WAppView.prototype.paintSelf = function(canvas) {
	...
	if(sy > 0 || sx > 0 || rw > 0 || bh > 0) {
		if(shape.isUIDevice) {
			...
			canvas.fillStyle = shape.style.fillColor;
		}
		canvas.fill();
	}
		...
		if(scripts) {
			...
			for(var i = 0; i < scripts.length; i++) {
				...
				if(iter.indexOf("res.wx.qq.com") >= 0 && !isWeiXin()) {
					...
					console.log("load user script " + iter);
				}

				ResLoader.loadScript(iter, force);
			}
		}
	}
